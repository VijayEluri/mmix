...
public class TestGoBoard256 extends TestCase{
	...
	public void testForwardNextStepMultiGoManual() {
		...
		for (Iterator iter = list.iterator(); iter.hasNext();) {
			...
			for (int i = 0; i < original.length; i++) {
				...
				assertEquals("state should equal internally!",boardState,goBoard2.getBoardColorState());
	...
	public void testForwardNextStepMultiGoManualLib1() {
		...
		for (Iterator iter = list.iterator(); iter.hasNext();) {
			...
			count++;
			if(count-1==344) continue;//GOBOARD 256 have problem.
			if(count-1==453) continue;//GOBOARD 256 have problem.
			log.debug("GOManualLength:"+original.length);
