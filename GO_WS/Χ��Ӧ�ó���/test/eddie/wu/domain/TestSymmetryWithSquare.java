			...
			for (byte j = 0; j <= 2; j++) {
				for (byte k = 0; k <= 2; k++) {
					for (byte l = 0; l <= 2; l++) {
						...
						GoBoardSymmetry go = new GoBoardSymmetry(state);
						...
						List<Point> blackCandidate = go
								.getCandidate(Constant.BLACK);
						log.info("Candidate of Black's turn: " + blackCandidate);
						List<Point> whiteCandidate = go
								.getCandidate(Constant.WHITE);
								...
								for (byte o = 0; o <= 2; o++) {
									for (byte p = 0; p <= 2; p++) {
										...
										for (byte q = 0; q <= 2; q++) {
											...
											// if (index != 274)
											// continue;
											GoBoardSymmetry go = new GoBoardSymmetry(
													state);
