...
public class Zhizuobianhua
   ...
   public void paint(Graphics g) {
      if(log.isDebugEnabled()) log.debug("进入方法paint()");
      if (CHONGHUI == true) {
      ...
      if(log.isDebugEnabled()) log.debug("从方法paint()返回");
   } //else画整个棋盘和棋子
   ...
   public boolean mouseDown(Event e, int x, int y) { //接受鼠标输入
      if (KEXIA == true) {
         ...
         repaint();
         if(log.isDebugEnabled()) log.debug("mousedown");
   ...
   byte fuzhi2(DsNode root) { //为各节点的形势赋值
      ...
      if (root.left == null) { //变化结束
         if(root.xingshi1!=root.xingshi2){
            //if(root.xingshi1 <0)
               //if(log.isDebugEnabled()) log.debug("征子成立也不好，不合理") ;
            root.ZZYG=true;
         }
   ...
   public boolean action(Event e, Object what) {
      ...
      if (e.target == wenjian) {
         ...
         if(rootb.xingshi1!=0&&rootb.xingshi2!=0){
            ...
            rootb.ju=ju;
            if(log.isDebugEnabled()) log.debug("该步是骗着：("+rootb.zba
                               +","+rootb.zbb+")");
         }
         else{//可以作为定式，能达到两分。
            ...
            //变化树文件。
            if(log.isDebugEnabled()) log.debug("该步可作为定式：("+rootb.zba
                             +","+rootb.zbb+")");
            if(rootb.ZZYG==true){
               if(log.isDebugEnabled()) log.debug("该步与征子有关");
            }
      ...
      if (e.target == store) {
         ...
         if (rootb == null) { //存储原始局面时的处理
            ...
            if (xds == true) {
               ...
               if(log.isDebugEnabled()) log.debug("该变化已经存在");
               showStatus("该变化已经存在");
            }
         }
         store.disable();
   ...
   void write(DataOutputStream out, DsNode root) {
      ...
      if(log.isDebugEnabled()) log.debug("局部root=(" + root.zba + "," + root.zbb + ")");
