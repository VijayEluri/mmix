...
import java.io.*;
import java.net.*;
public class Dingshiduikang extends Applet {
   boolean fdangbu=false;//悔棋时是否为单步。
   boolean fshuzi=false;//是否标出数字。
   boolean fbuhelidian=false;//制作时未予考虑的点。
   boolean ftishi=false;//是否需要提示。
   boolean fzhudong=false;//是否需要全部重画
   boolean fdonghua=false;//是否动态演示正解。
   int czhengjie=0;//正解的标志?
   int linss;//原始局面的手数。
   int chuiqi=0;// 悔棋的步数<3
   int cqianjin=0;// 前进的步数<3
   int [][][] huitemp=new int[2][50][2];//用于协调paint（）；
   int tsa=-1,tsb=-1;//提示的位置，用于删除提示标志。


   Tree gotree=new Tree();//存储变化的动态结构。
   TreeNode gotemp;//当前节点。
   ...
   public void init(){
      ...
      for(i=1;i<=linss;i++){
         ...
         if(a<0||b<0){
            ...
            hui[shoushu][25]=-1;
            hui[shoushu][26]=-1;
            System.out.print("dian:a=" +a+",b="+(b+1)+"=qi quan 1 shou");
         }else{
            cgcl();
         }
      }
      ...
      if (a>=0&&a<=18&&b>=0&&b<=18&&zb[a][b][2]==0){
         ...
         }else
         {
           ...
           if(a==gotemp.zba&&b==gotemp.zbb){
               czhengjie-=1;//?wei le pei he hou mian suan fa .bu hao li jie
            }
           while(gotemp!=null){
              if(a==gotemp.zba&&b==gotemp.zbb){
                  a=gotemp.zba;
           ...
           if(gotemp==null){
              ...
              if (gotemp==null){
                  ...
                  cqianjin++;
                if(s!=null) {
                   if(jieshuo.getText()!=null) s=jieshuo.getText()+"\n"+s;
                     jieshuo.setText(s);
                  }
                  old=gotemp;
                  gotemp=gotemp.left;
...
//10.1ji suan kuai qi.
   ...
   public void xz(Graphics g,int a,int b){//12.1
      ...
      g.setColor(Color.black);
      if(a==0){
           g.drawLine(bjjx+qpbj,qpdx*b+bjjx+qpbj,bjjx+qpdx,qpdx*b+bjjx+qpbj);
      }
      else if(a==18){
           g.drawLine(qpdx*a+bjjx,qpdx*b+qpbj+bjjx,qpdx*a+qpbj+bjjx,qpdx*b+qpbj+bjjx);
      }else {
           g.drawLine(qpdx*a+bjjx,qpdx*b+qpbj+bjjx,qpdx*a+qpdx+bjjx,qpdx*b+qpbj+bjjx);
      }
      if(b==0){
          g.drawLine(qpdx*a+qpbj+bjjx,qpbj+bjjx,qpdx*a+qpbj+bjjx,bjjx+qpdx);
      }else if(b==18){
          g.drawLine(qpdx*a+qpbj+bjjx,qpdx*b+bjjx,qpdx*a+qpbj+bjjx,qpdx*b+qpbj+bjjx);
      }else{
          g.drawLine(qpdx*a+qpbj+bjjx,qpdx*b+bjjx,qpdx*a+qpbj+bjjx,qpdx*b+bjjx+qpdx);
      }
      if((a-3)%6==0&&(b-3)%6==0){
         g.fillOval(qpdx*a+bjjx+qpbj-xbj,qpdx*b+bjjx+qpbj-xbj,xbj*2+1,xbj*2+1);
...
class DsNode {//定式节点。
   ...
   DsNode right;
   public DsNode(byte a,byte b,byte pj) {
      left=null;
      right=null;
