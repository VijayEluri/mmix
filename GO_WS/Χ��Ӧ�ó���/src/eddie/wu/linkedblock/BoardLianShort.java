...
public class BoardLianShort implements Cloneable {
	...
	public boolean validate(byte a, byte b) { // 落子位置的有效性。
		...
		if (a > ZBXX && a < ZBSX && b > ZBXX && b < ZBSX
			...
			if (a == hui[shoushu][3] && b == hui[shoushu][4]) { // 是否禁着点
				...
				// if(CS.DEBUG_CGCL) System.out.println("落点为：a=" + a + ",b=" +
				// b);
				for (byte i = 0; i < 4; i++) {
					m = (byte) (a + szld[i][0]);
					n = (byte) (b + szld[i][1]);
	...
	public void zjq(byte a, byte b, byte tiao) { // function 6.1
		...
		for (i = 0; i < 4; i++) {
			...
			if (zb[m1][n1][ZTXB] == tiao) {
				 ...
				 * 1) { if(CS.DEBUG_CGCL) System.out.println("悔棋时气数出错:a=" + m1 +
				 * ",b=" + n1); } }
				 */
				// else {
				for (j = 0; j < yiseks; j++) {
		...
		for (byte i = 1; i <= kuaishu; i++) {
			...
			if (qikuai[qikin].color != 5) {
				...
				for (byte j = 1; j <= qikuai[qikin].zishu; j++) {
					...
					if (CS.DEBUG_CGCL) {
						System.out.println("气点为：" + linshidian.a + "/"
								+ linshidian.b);
					}
					if (zijishu >= 39) {
						fanhui[0][0] = 39;
	...
	public byte jisuansihuo(byte a, byte b, boolean xianhou) {
		...
		while (true) {
			...
			// 新层的开始点。
			if (CS.DEBUG_CGCL) {
				System.out.println("新层的开始局面索引为：" + (jumianshu + 1));
			...
			if (st[cengshu][1] == MAX) {
				...
				for (byte i = 1; i <= houxuan[0][0]; i++) {
					...
					if (temp.validate(m1, n1)) { // 判断合法着点
						...
						youxiaodian++;
						if (CS.DEBUG_JISUANZHENGZI) {
							System.out.println("第" + youxiaodian + "个候选点为:("
									+ m1 + "," + n1 + ")");
						}
	...
	public void cgcl(byte a, byte b) { // chang gui chu li
		...
		// 已经扣除落子点。
		if (CS.DEBUG_CGCL) {
			System.out.println("落子点所在气块子数现为：" + yuanqikuaizishu);
			// 原气块落子后的子数；
		}
		...
		if (CS.DEBUG_CGCL) {
			System.out.println("落子点周围的原始直接气点为：" + yuanshizhijieqishu);
			...
			if (zb[m1][n1][ZTXB] == yise) { // 1.1右边相邻点
				...
				if (j == yiseks) { // 不重复
					...
					if (qi == 0) { // 原来的气。
						...
						if (CS.DEBUG_CGCL) {
							System.out.println("块" + kin1 + "气数减为"
									+ zb[m1][n1][QSXB]);
						}
						if (qi == 1) {
		...
		case 2: {
			...
			if (m1 == m2 || n1 == n2) {
				...
				if (CS.DEBUG_CGCL) {
					System.out.println("chengqikuai()=" + ttt
	 ...
	 * block index. if(CS.DEBUG_CGCL) System.out.println("方法:处理悔棋(clhuiqi)\n");
	 ...
	 * if(CS.DEBUG_CGCL) System.out.println("悔棋:" + shoushu); if(CS.DEBUG_CGCL)
	 * System.out.println("a=" + m + ",b=" + n); kin = hui[shoushu][0]; if (kin
	 ...
	 * System.out.println("//计算成块点的气:" + "a=" + m + ",b" + n); } } //deal with 3
	 ...
	 * System.out.println("a=" + m + ",b=" + n); } } //for for (i = 1; i <= 4;
