...
/**
 * <p>Title: 棋盘局面的基础信息都在数组zb中，本类从BoardLianShort
 * 转化而来，专门处理从静态局面生成高级信息。</p>
 * <p>Description: </p>
...
public class JuMian3 {
  ...
  public byte jskq(short rs) {
    ...
    byte m, n;
    byte i, j;
    short zishu = kuai[rs].zishu; //块的子数
  ...
  public void output() { //输出每块的信息;
    ...
    for (i = 1; i <= qiki; i++) {
      ...
      System.out.println("块号:" + i + "；zishu:" + qikuai[i].zishu);
  ...
  public void cgcl(byte a, byte b) { //chang gui chu li
    ...
    for (i = 0; i < 4; i++) { //再处理同色邻子
      ...
      if (zb[m1][n1][CS.ZTXB] == tongse && zbk[m1][n1] != ki) { //3.1
        ...
        HaoNode1 teli = kuai[kin1].zwyskhao; //修改周围块号的指针

        for (byte p = 1; p <= kuai[kin1].zwyskshu; p++) {
          kuai[teli.hao].deleteyisekuaihao(kin1);
          kuai[teli.hao].addyisekuaihao(ki); //修改块号
    ...
    for (i = 0; i < 4; i++) { //先处理异色邻子
      ...
      m1 = (byte) (a + CS.szld[i][0]);
      n1 = (byte) (b + CS.szld[i][1]);
      if (zb[m1][n1][CS.ZTXB] == yise) { //1.1右边相邻点
        kys++; //异色点计数
        kin1 = zbk[m1][n1]; //块索引
        //if (kin1==0)为块。
        for (j = 0; j < yiseks; j++) {
        ...
        if (j == yiseks) { //不重复
          ...
          if (qi == 0) { //原来的气。
            ...
            kuai[ki].addyisekuaihao(kin1); //要防止重复
            System.out.println("块" + kin1 + "气数减为" + zb[m1][n1][CS.QSXB]);
            if (qi == 1) {
      ...
      case 2: {
        ...
        if (m1 == m2 || n1 == n2) {
          ...
          if (zb[x][y][CS.ZTXB] == CS.BLANK) {
            System.out.println("直接气数为2，对角点为空，没有新块生成。");
          }
          else if (jgzs(x, y) == 1) {
            System.out.println("直接气数为2，对角点周围空，没有新块生成。");
          }
          else {
...
//bianli
        ...
        if (m2 == m3 || n2 == n3) {
          ...
          if (zb[x][y][CS.ZTXB] == CS.BLANK) {
            ...
            //bianli.
            System.out.println("直接气数为3，可能有新块生成。");
      ...
      case 4: {
       ...
       if(lianjieshu>=3){
         System.out.println("直接气数为4，lianjieshuwei3，没有新块生成。");
  ...
  /* public void clhuiqi() { //是否所有数据结构都能恢复?
     ...
     System.out.println("方法:处理悔棋(clhuiqi)\n");
  ...
  public byte bijiaoqishu(
    ...
    //从另一方调用就是0：对杀失败，也可
    //看成弃子，先手利用；
    //一方调用为2：本方脱先仍然获胜(已胜)；但是脱先后对方有先手利用；
    //从另一方调用就是－1：对方脱先仍然失败；
    //一方调用为3：本方脱先仍可获胜（无劫材已胜），且脱先后对方没有利用。
  ...
  public boolean chengqikuai(byte a, byte b) {
    //收集信息的过程中,可以令color=CS.BLANK,调用该函数,
    //但是气块的信息
    ...
    if (qkzishu < 10) {
      ...
      qkzishu++;
      System.out.println("该块的子数超过10,块号为:" + ki);
      return false;
    }
    zb[a][b][CS.SQBZXB] = 1;
  ...
  //最终状态的确定。如果没有最起码的智能，一盘棋甚至可以下到非常大的步数；
  //比如，胜了的棋仍然可以自己填眼而死掉，就象后手死之类。这种情况下，完全
  //博弈树的理论就有缺陷，比如结束不了，当然也可能通过禁止局面循环来解决。
}
