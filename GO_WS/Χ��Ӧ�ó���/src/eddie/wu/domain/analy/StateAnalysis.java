...
import eddie.wu.domain.BreathPattern;
 ...
 * 
 */
public class StateAnalysis extends GoBoardSymmetry {
	private static final Logger log = Logger.getLogger(StateAnalysis.class);
	...
	public void collectSpecialPoint() {
		...
		for (Point point : Point.getAllPoints(boardSize)) {
			if (this.getColor(point) == ColorUtil.BLANK) {
				...
				TigerMouth tmAdj = this.isTigerMouth_(tm.getSeek());
				if (tmAdj == null) {
	 ...
	 * @return
	 */
	public BreathPattern toBreathPattern(BlankBlock breathBlock) {
