...
//		}
		...
		while (true) {
			...
			if (st[cengshu][1] == MAX) {
				...
				for (byte i = 1; i <= houxuan[0][0]; i++) {
					...
					if (temp.validate(m1, n1)) { // 判断合法着点
						temp.oneStepForward(Point.getPoint(boardSize,m1, n1),blockToBeEaten.getColor());
						youxiaodian++;
						if (log.isDebugEnabled()) {
			...
			// zheng zi fang zou.
				...
				for (byte i = 1; i <= houxuan[0][0]; i++) {
					...
					// 扩展最后的局面，扩展同一个上级局面。
					if (temp.validate(m1, n1)) { // 判断合法着点
						temp.oneStepForward(Point.getPoint(boardSize,m1, n1),ColorUtil.enemyColor(blockToBeEaten.getColor()));
						youxiaodian++;
	...
	public GoBoardLadder getGoBoardLadderCopy() {
		if(log.isDebugEnabled()) log.debug("this=" + this + " shoushu = " + shoushu);
		if(log.isDebugEnabled()) log.debug(getStepHistory().getAllSteps());
		...
		if(log.isDebugEnabled()) log.debug(temp.getStepHistory().getAllSteps());
