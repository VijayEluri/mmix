 ...
 * 
 */
public class GoBoardForward extends BasicGoBoard {
	...
	public boolean oneStepForward(final Step step) {
		...
		if (validate(original, selfColor) == false) { // 1.判断落子点的有效性。
	...
	private void dealBlankPoint(NeighborState state) {
		...
		if (this.isConnectedWithoutPoint(state, blankBlock, original)) {
			...
			if (blankBlock.getPoints().size() == 0) {
				if (log.isInfoEnabled())
		...
		this.stepHistory.getLastStep().setDividedBlock(blankBlock);
	...
	private void dealEatenBlock(Point original, Block eatenBlock) {
		...
		blankBlock.setPoints(eatenBlock.getPoints());
	...
	private Block dealSelfPoint(NeighborState neighborState) {
		...
		int friend = neighborState.getFriend();
		Block sameColorBlock = null;

		if (friend >= 2) {
			if (log.isInfoEnabled()) {
				log.info("一.1 建立初始新子块, " + friend + " 块被合并。");
			}
	 ...
	 * @param jubutizishu
	 */
	private void dealJie(Point point, byte jubutizishu) {
		if (getBlock(point).getBreaths() == 1 && jubutizishu == 1
			...
			if (breathBlocks.size() == 1) {
				...
				temp.setProhibitStep((short) (shoushu));
				// 将互为打劫的两点联系在一起.
	...
	private void realDivideBlankPointBlock(NeighborState state) {
		...
		if (countNewBlank > 1) {
			log.warn("New blank block generated:" + newBlank.getTopLeftPoint());
		} else
