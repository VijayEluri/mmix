...
/**
 * @author eddie
 * resolve the array out of bound problem of blocks.
 * 
 * 1. using Log4J to Debug
 * 2. compare between different implementation --goog and close
 * 	  begin to compare internally.
 * 
...
public class GoBoard256 {
	...
	byte ktz = 0; //提子计数,局部
	byte numberOfNeighbourBlock = 0, numberOfNeighbourPoint = 0; //ks is count for block,kss for single point
	//相邻的成块点数和独立单点数
	short kin1 = 0;
	...
	public void cgcl(final byte row,final byte column) { //chang gui chu li qi zi de qi
...
//		byte n1 = column; //m1,n1为a,b的邻点.
		...
		//三种点的计数
		 ...
		 numberOfNeighbourPoint = 0; //ks is count for block,kss for single point
		//相邻的成块点数和独立单点数
//		short kin1 = 0;
		 m = 0;
		 n = 0; //the block index.
		...
		short[] neighbourSelfBlock = { 0, 0, 0, 0 }; //array for block index.四同色邻子的块索引
		...
		dealWithSelfPoint(row, column, neighbourPointRow, neighbourPointColumn, neighbourSelfBlock);
		...
		if (numberOfSelfPoint == 0) { //4.1 no same color pobyte surround没有同色邻点
			log.debug("//k3=0");
			dealWithNoSelfPoint(row, column, neighbourPointRow, neighbourPointColumn);
			return;
		}

		if (numberOfNeighbourBlock == 0) { //4.2 only single pobyte surr.有同色点,但都为独立点
			log.debug("//ks=0");
			dealWithNoSelfBlock(row, column, neighbourPointRow, neighbourPointColumn);
		}

		if (numberOfNeighbourBlock > 0) { //相邻有块
			log.debug("//ks>0"+numberOfNeighbourBlock);
			dealWithMultiSelfBlock(row, column, neighbourPointRow, neighbourPointColumn, neighbourSelfBlock);
			/*
	 ...
	 */
	private void dealWithMultiSelfBlock(final byte row, final byte column, byte[] neighbourPointRow, byte[] neighbourPointColumn, short[] neighbourSelfBlock) {
		ki++;
		...
		for (j = 1; j <= numberOfNeighbourBlock; j++) {
			...
			kkhb(ki, neighbourSelfBlock[j - 1]); //not deal with breath块块合并,气尚未处理.
		}
		// hui[shoushu][0]=ki;
		//zb[a][b][2]=tongse;
	 ...
	 * @param neighbourPointColumn
	 */
	private void dealWithNoSelfBlock(final byte row, final byte column, byte[] neighbourPointRow, byte[] neighbourPointColumn) {
		gq = 0;
		for (i = 1; i <= numberOfNeighbourPoint; i++) { //4.1 deal surr point处理相邻独立点
			...
			for (j = 1; j <= (numberOfNeighbourPoint - i); j++) { //计算点之间的共气
				gq += dd(neighbourPointRow[k0 + i], neighbourPointColumn[k0 + i], neighbourPointRow[k0 + i + j], neighbourPointColumn[k0 + i + j]);
			}
		}
		zb[row][column][QSXB] = (byte) (dang - gq);
		zb[row][column][KSYXB] = (byte) (++ki-MAX_BYTE); //count from first block
		//TODO:change code of huiqi()
	 ...
	 * @param neighbourPointColumn
	 */
	private void dealWithNoSelfPoint(final byte row, final byte column, byte[] neighbourPointRow, byte[] neighbourPointColumn) {
		byte ktm;
		byte ktn;
	 ...
	 * @param neighbourSelfBlock
	 */
	private void dealWithSelfPoint(final byte row, final byte column, byte[] neighbourPointRow, byte[] neighbourPointColumn, short[] neighbourSelfBlock) {
		byte m1;
		byte n1;
		short kin1;
		//根据成块算法统计子数,看是否生成新块
		for (i = 0; i < 4; i++) { //再处理同色邻子
			...
			if (zb[m1][n1][ZTXB] == tongse) { //3.1
				...
				if (kin1 == 0) { //独立点
					...
					neighbourPointRow[k0 + numberOfNeighbourPoint] = m1; //u[0] not used
					neighbourPointColumn[k0 + numberOfNeighbourPoint] = n1; //deal with single point.
				} else { //231成块点
					...
					if (j == numberOfNeighbourBlock) { //不重复
						...
						dang--;
						neighbourPointRow[neighbourPoints - numberOfNeighbourBlock] = m1; //deal with block.
						neighbourPointColumn[neighbourPoints - numberOfNeighbourBlock] = n1;
	 ...
	 * @param neighbourPointColumn
	 */
	private void dealWithBlankPoint(final byte row, final byte column, byte[] neighbourPointRow, byte[] neighbourPointColumn) {
		byte m1;
		byte n1;
	 ...
	 */
	private void dealWithEnemyPoint(final byte row, final byte column, short[] neighbourEnemyBlock) {
		byte m1;
		byte n1;
		short kin1;
		for (i = 0; i < 4; i++) { //先处理异色邻子
			...
			if (zb[m1][n1][ZTXB] == yise) { //1.1右边相邻点
				...
				kin1 = (short)(zb[m1][n1][KSYXB]+MAX_BYTE); //the block index for the point.66
