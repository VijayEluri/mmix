...
public class GoBoard2 {
	...
	//前两维是棋盘的坐标,数组下标从1到19;
	public byte[][][] kuai = new byte[128][70][2]; //17k;
	...
	public void cgcl(byte a, byte b) { //chang gui chu li qi zi de qi
		 ...
		 * zb[u[k0+3]][v[k0+2]][ZTXB]==BLANK){ System.out.println("三气全相连"); }
		...
		if (ks == 0) { //4.2 only single pobyte surr.有同色点,但都为独立点
			...
			if (zb[a][b][2] != jskq(ki)) {
				System.out.println("error of breath");
			}
		}
	...
	public void chengkuai(byte a, byte b, byte color) {
		...
		if (zishu < 49) { //防止子数太多。
			...
			System.out.println("该块的子数超过49,块号为:" + ki);
		}
		zb[a][b][SQBZXB] = 1;
		zb[a][b][KSYXB] = ki;
		//zishu++;
	...
	public void output(byte kin) { //打印棋块的子数，各点坐标
		...
		System.out.println("\n块号为：" + kin);
		for (i = 1; i <= p; i++) {
			System.out.print("" + kuai[kin][i][0] + "," + kuai[kin][i][1]
					+ "\t");
			if (i == 5) {
				System.out.println();
			}
		...
		System.out.println("\n气数为：" + qishu);
		for (i = 1; i <= qishu; i++) {
			System.out.print("" + kuai[kin][49 + i][0] + ","
					+ kuai[kin][49 + i][1] + "\t");
			if (i == 5) {
				System.out.println();
			}
	...
	public GoBoard2(GoBoard2 goold) {
