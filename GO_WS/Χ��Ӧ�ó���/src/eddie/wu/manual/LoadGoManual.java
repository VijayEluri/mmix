...
public class LoadGoManual {
	...
	private static final String ROOT="C:/scm/github/mmix/GO_WS/围棋应用程序/doc/围棋打谱软件/";
	private static final String GO_MANUAL_DATA_FILE_LOCATION = ROOT+"vol000";
	...
	public static byte[] loadSingleGoManual() {
	 ...
	 * @return
	 */
	public static List loadMultiGoManual(int libId) {
		...
		return loadMultiGoManual(GO_MANUAL_DATA_FILE_LOCATION+libId+".gmd", numberOfGoManuals);
		
	}

	/**
	 * return a series of go manual
	 * @param goManualLibFileName name of lib
	 * @param numberOfGoManuals total number of go manual
	 * @return
	 */
	private static List loadMultiGoManual(String goManualLibFileName,int numberOfGoManuals) {
		List<byte[]> list = new ArrayList<byte[]>(1024);
		DataInputStream in = null;

		byte a, b;
		byte[] original = new byte[ByteLengthOfSingleGoManual * numberOfGoManuals];
		byte[] standard = null;
		try {
			...
			int shoushuCount = 0;
			int errorCode = in
					.read(original, 0, ByteLengthOfSingleGoManual * numberOfGoManuals);
		
			for (int goManualCount = 0; goManualCount < numberOfGoManuals; goManualCount++) {
				log.debug("ii=" + goManualCount);
				int shoushu = original[SHOUSHUINDEX+goManualCount*ByteLengthOfSingleGoManual];
				int shoushuHigh = original[SHOUSHUINDEX+goManualCount*ByteLengthOfSingleGoManual+1];
				...
				for (int i = 0; i < shoushu * 2; i++) {
					shoushuCount++;
					a = original[FIRSTSTEPINDEX+goManualCount*ByteLengthOfSingleGoManual + i];
					a++;
					...
					standard[i+1] = a;
					i++;
					b = original[FIRSTSTEPINDEX+goManualCount*ByteLengthOfSingleGoManual + i];
					b++;
					standard[i-1] = b;
					//standard[i] = b;
...
//						log.debug(",j=" + b);
	 ...
	 * @return
	 */
	public static List loadMultiGoManualFromLib0() {
		return loadMultiGoManual(ROOT+"vol0000.gmd",9);
		
	}
	/**
	 * get any go manaul from lib 0
	 * 0--9
	 * @param id
	 * @return
	 */
	public static byte[] loadOneFromLib0(int manualid) {
	...
	/**
	 * load one go manual from one lib
	 * 0 --9
	 * 1-6 --1024
	 * 7---382
	 * @param libid
	 ...
	 * @return
	 */
	public static byte[] loadOneFromAllGoManual(int libid,int manualid) {
		int shu=0;
		if(libid>=0&&libid<=7){
			if(libid==0){
				if(manualid>=0&&manualid<9){
		...
		List list = loadMultiGoManual(GO_MANUAL_DATA_FILE_LOCATION+libid+".gmd",shu);
		return (byte[])list.get(manualid);		
	}
