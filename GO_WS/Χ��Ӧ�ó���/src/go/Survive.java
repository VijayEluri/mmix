 ...
 * 
 */
public class Survive extends StateAnalysis {

	private static final Log log = LogFactory.getLog(Survive.class);

	public Survive(byte[][] state) {
		super(state);
	...
	public boolean isOneBlockTwoEyeLive(Point point) {
		...
		if (log.isWarnEnabled()) {
			log.warn("拥有的真眼为: " + Arrays.toString(eyes.toArray()));
		}
		if (eyes.size() < 2) {
			return false;
		} else {
