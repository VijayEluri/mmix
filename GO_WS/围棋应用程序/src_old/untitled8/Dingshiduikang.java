...
public class Dingshiduikang
   ...
   Button zhiding = new Button("指定局面");
   ...
   public boolean mouseDown(Event e, int x, int y) { //接受鼠标输入
      ...
      if (KEXIA == true) {
         ...
         if(log.isDebugEnabled()) log.debug("落点相当于："+m+","+n);
         if(work==null){
            if(log.isDebugEnabled()) log.debug("定式中该变化到此结束。");
         ...
         if(temp==null){
            if(log.isDebugEnabled()) log.debug("定式中没有该变化。");
            showStatus("定式中没有该变化。");
            go.clhuiqi() ;
            KEXIA=true;
            return true;
         ...
         if(work==null){
            ...
            m=work.zba;
            n=work.zbb;
            if(log.isDebugEnabled()) log.debug("选择的变化为："+m+","+n);
            if (fuan == 2) {
               t = m;
               m = n;
               n = t;
