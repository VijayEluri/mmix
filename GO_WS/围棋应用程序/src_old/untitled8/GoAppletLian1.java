...
public class GoAppletLian1
   ...
   public void init() {
      if(log.isDebugEnabled()) log.debug("进入方法init()");
      add(zzwb);
      add(zhzi);
      add(houtui);
      ...
      if(log.isDebugEnabled()) log.debug("从方法init()返回");
   }

   public boolean action(Event e, Object what) {
      if(log.isDebugEnabled()) log.debug("进入方法action()");
      if (e.target == zhzi) {

         goji = new JisuanBoardLian(goboard);
         //if(log.isDebugEnabled()) log.debug("方法getText()");
         byte kkk = (byte) Integer.parseInt(zzwb.getText().toString());
         if(log.isDebugEnabled()) log.debug("被征子的块为："+kkk);
         boolean acti = goji.yiqichi(kkk);
         //if(log.isDebugEnabled()) log.debug("方法getText()");
         if (acti == true) {
            if(log.isDebugEnabled()) log.debug("征子成立");
         }
         if(log.isDebugEnabled()) log.debug("从方法action()返回");
         ZZ = true;
         CHONGHUI = true;
         repaint();
      }
