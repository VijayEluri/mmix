...
{private static final Logger log = Logger.getLogger(QiKuai1.class);
   ...
   public void init(){//初始化,完成界面
      int i,j;
       ...
       /* try{
         ...
         if(log.isDebugEnabled()) log.debug("documentBase="+dbase);
        URL urlbase=new URL( base+"go");
        if(log.isDebugEnabled()) log.debug("UrlBase="+urlbase);
       ...
       if ((a+1)<=18&&zb[a+1][b][0]==yise) {//1.1右边相邻点
            ...
            if (kin1==0){      //not a block.不是块
               ...
               if(zb[a+1][b][2]<=0){//eat the diff point
                  ...
                  hui[shoushu][tzd*2]=b;
                  if(log.isDebugEnabled()) log.debug("ti zi 1 xing");
         ...
         if((a-1)>=0&&zb[a-1][b][0]==yise) {//1.2
            ...
            if (kin2==0){//99
               ...
               if(zb[a-1][b][2]<=0){
                  ...
                  hui[shoushu][tzd*2-1]=a-1;
                  hui[shoushu][tzd*2]=b;
                  if(log.isDebugEnabled()) log.debug("ti zi 2 xing");
                  ktz++;
                  zzq(a-1,b,tongse);//zi zhen qi
               }
            }
         ...
         if((b+1)<=18&&zb[a][b+1][0]==yise) {//1.3
            ...
            if (kin3==0){
	          ...
	          k1--;
                  ...
                  if(log.isDebugEnabled()) log.debug("ti zi 3 xing");
                  ktz++;
         ...
         if((b-1)>=0&&zb[a][b-1][0]==yise){//1.4
            ...
            if (kin4==0){
               ...
               if (zb[a][b-1][2]<=0){//165
                  ...
                  hui[shoushu][tzd*2-1]=a;
                  hui[shoushu][tzd*2]=b-1;
                  if(log.isDebugEnabled()) log.debug("ti zi 4 xing");
                  ktz++;
                  zzq(a,(b-1),tongse);//zi zhen qi
               }
            }
      ...
      if(fzhudong==true){
        ...
        if(fqianjin==true){
           ...
           if(tongse==1) g.setColor(Color.black);
            ...
            if(log.isDebugEnabled()) log.debug("color="+tongse);
            g.fillOval(28*m+4,28*n+4,28,28)  ;
            for(i=1;i<=4;i++){
	 ...
	 g.setColor(Color.orange);
      ...
      for (i=10;i<19;i++){
        for ( j=0;j<10;j++){
           ...
           if(zb[j][i][2]>0){
               if(log.isDebugEnabled()) log.debug("zb["+j+"]["+i+"][2]="+zb[j][i][2]);
            }
         }
      }
      ...
      for( xun=linss+1;xun<=shoushu;xun++){
         ...
         //if(log.isDebugEnabled()) log.debug("//come into the end of method paint");
   }
            ...
            DataInputStream  fnin=new DataInputStream(
                 new BufferedInputStream(
                    new FileInputStream("c:\\sihuoti\\"+"qipushu")));
                       ju=  fnin.readInt() ;
                       if(log.isDebugEnabled()) log.debug("ju="+ju);
                       ju++;
                      fnin.close() ;
  ...
  fnout.close() ;
               ...
               if(log.isDebugEnabled()) log.debug("the reading of file  meet some trouble!");
            ...
            try{
             ...
             if(log.isDebugEnabled()) log.debug("bhs="+bhs);
            for(i=0;i<bhs;i++){
                   p=bianhua[i][0][0];
                   if(log.isDebugEnabled()) log.debug("shoushu="+p);
                 for(j=0;j<=p;j++){
            ...
            /*  while(lins<shoushu){
                   ...
                   if(log.isDebugEnabled()) log.debug("a="+hui[lins][25]);
              }
              out.writeByte(100);
      ...
      try{
          DataOutputStream out2=
             new DataOutputStream(
               ...
               for(i=0;i<jss;i++){
                 s=tempstr[i]+"\n";
                if(s==null)   if(log.isDebugEnabled()) log.debug("eror");
       ...
       if(e.target==jiarujieshuo){
          s=jieshuo.getText();
          if(log.isDebugEnabled()) log.debug("jshuo["+bhs+"]["+(shoushu-linss)+"]=\""+s+"\";");
          jsweizhi[jss][0]=bhs;
       ...
       if(e.target==store)
       {          //if(log.isDebugEnabled()) log.debug("come into method store!");
          if(bhs==0){
              //if(log.isDebugEnabled()) log.debug("lins="+shoushu);
             ...
             for(i=1;i<=shoushu;i++){
                  m=hui[i][25];
                  n=hui[i][26];
    if(log.isDebugEnabled()) log.debug("bianhua[0]["+i+"][0]="+m+";");
    if(log.isDebugEnabled()) log.debug("bianhua[0]["+i+"][1]="+n+";");
                  bianhua[bhs][i][0]=m;
                  bianhua[bhs][i][1]=n;
              }
              if(log.isDebugEnabled()) log.debug("bianhua["+bhs+"][0][0]="+shoushu+";");
             ...
             pass.setEnabled(false);
               ...
               for(i=1;i<=shoushu-linss;i++){
                  m=hui[i+linss][25];
                  n=hui[i+linss][26];
    if(log.isDebugEnabled()) log.debug("bianhua["+bhs+"]["+i+"][0]="+m+";");
    if(log.isDebugEnabled()) log.debug("bianhua["+bhs+"]["+i+"][1]="+n+";");
                  bianhua[bhs][i][0]=m;
                  bianhua[bhs][i][1]=n;
              }
    if(log.isDebugEnabled()) log.debug("bianhua["+bhs+"][0][0]="+(shoushu-linss)+";");
             bianhua[bhs][0][0]=shoushu-linss;
           }
           if(log.isDebugEnabled()) log.debug("bianhua[0][0][1]="+bhs+";");
           bianhua[0][0][1]=bhs;
           bhs+=1;
           store.setEnabled(false) ;
...
//10.1ji suan kuai qi.
   ...
   public void clhuiqi(){
     ...
     // int dang=0,ktz=0;//,kq=0,p=0,q=0; //dang is breath of block.
      ...
      fhuiqi=true;
      zzq(m,n,yise);
      if(log.isDebugEnabled()) log.debug("//regret step:"+shoushu);
      if(log.isDebugEnabled()) log.debug("a="+m);
