 ...
 * 
 */
public class CopyOfGoBoardForward extends BasicGoBoard {
	public CopyOfGoBoardForward(int boardSize) {
		super(boardSize);
	}

	private transient static final Logger log = Logger
			.getLogger(CopyOfGoBoardForward.class);
	...
	public boolean oneStepForward(final Step step) {
		...
		/**
		 * 生成新的棋块，比如将邻块并入，但是气数尚未计算(现在直接维护相邻关系,已经无需计算气数)。
		 */
		...
		// 计算sameColorBlock的breathBlock情况.
		...
		if (step.isLoopSuperior()) {
			...
			if(log.isEnabledFor(Level.WARN)) log.warn("模拟找了绝对劫材之后提回。避开全局同型检查。");
		} else {
			// prevent duplicate state.
			boolean valid = stepHistory.setColorState(getBoardColorState());
			if (valid == false) {
				if(log.isEnabledFor(Level.WARN)) log.warn("落子点无效:全局同型再现!");
	...
	protected void check_internal(String messageIn) {
		if(log.isEnabledFor(Level.WARN)) log.warn(messageIn);
		for (BasicBlock basicBlock : this.getAllBlocks()) {
			/**
			 * 块中的点确实指向该块
			 */
			for (Point point : basicBlock.allPoints) {
				Assert.assertEquals("check point" + point, basicBlock,
				...
				for (Block neighborBlock : blankBlock.getNeighborBlocks()) {
					...
					for (Point point : neighborBlock.allPoints) {
						if (neighborBlock != this.getBasicBlock(point)) {
							result = false;
							if (result == false) {
								if(log.isEnabledFor(Level.WARN)) log.warn("check block: " + blankBlock);
								if(log.isEnabledFor(Level.WARN)) log.warn("its neighorBlock" + neighborBlock
										+ " is wrong for point" + point);
								if(log.isEnabledFor(Level.WARN)) log.warn("correct one"
										+ this.getBasicBlock(point));
							}
							Assert.assertEquals(
									"check " + basicBlock.getBehalfPoint()
				...
				for (Point breath : block.getBreathPoints()) {
					...
					if (block.getBreathBlocks().contains(
						...
						String message = "no breath block for breath" + breath
								+ "at block" + block;
						if(log.isEnabledFor(Level.WARN)) log.warn(message);
						throw new RuntimeException(message);
					}
				}
				if (breathBlocks.isEmpty() == false) {
					// 防止breathBlocks中有多余的块。
					if(log.isEnabledFor(Level.WARN)) log.warn(breathBlocks);
					throw new RuntimeException("Extra blank block at Block "
				...
				for (Block enemyBlock : block.getEnemyBlocks()) {
					...
					for (Point point : enemyBlock.allPoints) {
						if (enemyBlock != this.getBasicBlock(point)) {
							result = false;
							if (result == false) {
								if(log.isEnabledFor(Level.WARN)) log.warn("check block: " + block);
								if(log.isEnabledFor(Level.WARN)) log.warn("its enemyBlock" + enemyBlock
										+ " is wrong for point" + point);
							}
	...
	public BoardColorState getBoardColorState() {
		...
		if (this.stepHistory.getAllSteps().isEmpty()) {
			boardState = new BoardColorState(boardSize, -1);
		} else {
			int color = this.getLastStep().getColor();
			int whoseTurn = ColorUtil.enemyColor(color);
			boardState =  BoardColorState.getInstance(this.getColorArray(), whoseTurn);
		}
	...
	private void dealBlankPoint(NeighborState state) {
		...
		if (this.isConnectedWithoutPoint(state, blankBlock, original)) {
			this.getLastStep().setOriginalBlankBlock(blankBlock);

			if (blankBlock.getPoints().size() == 1) {
				if (log.isInfoEnabled())
				...
				for (Block neighbourBlock : blankBlock.getNeighborBlocks()) {
					...
					// remove the disappeared block from its neighbors.
					if (neighbourBlock.getBreathBlocks().isEmpty()) {
						continue;
					}
					boolean removed = neighbourBlock.getBreathBlocks().remove(
							blankBlock);
					if (removed == false)
						throw new RuntimeException(
								"blankBlock is refered, cannot be removed"
										+ blankBlock + "original block"
										+ neighbourBlock);
				}
				// 消失的气块保留一个点，用于显示。
		...
		for (Block neighbourBlock : blankBlock.getNeighborBlocks()) {
			if (neighbourBlock.getBreathBlocks().isEmpty()) {
				if(log.isEnabledFor(Level.WARN)) log.warn("BlankBlock" + blankBlock.getBehalfPoint()
						+ " has already be removed form its neighbours"
						+ neighbourBlock);
				continue;
			}

			/**
			 * single direction update.<br/>
			 * 原始的未分裂气块仍旧指向这些相邻棋块 (只是这些棋块不再指向该原始气块)
			 * 
			 */
			removed = neighbourBlock.getBreathBlocks().remove(blankBlock);
			if (removed == false) {
				if(log.isEnabledFor(Level.WARN)) log.warn("blankBlock: " + "@" + Integer.toHexString(hashCode()));
				for (BlankBlock bb : neighbourBlock.getBreathBlocks()) {
					if(log.isEnabledFor(Level.WARN)) log.warn("blankBlock list: " + "@"
							+ Integer.toHexString(hashCode()));
				}
				throw new RuntimeException(
						"blankBlock is refered, cannot be removed" + blankBlock
								+ "original block" + neighbourBlock);
			} else {
				if(log.isEnabledFor(Level.WARN)) log.warn("breath block" + blankBlock.getBehalfPoint()
						+ " was removed from Block"
		...
		/**
		 * add the original blank block back. note is has code is changed. if
		 * the block is divided, new block is assigned during dividing.
		 */

		// if (newBlockCreated == false) {
		// for (Block neighbourBlock : blankBlock.getAllBlocks()) {
		// neighbourBlock.addBreathBlock(blankBlock);
		// }
		// } else {
	...
	private byte dealEnemyPoint(NeighborState state, Block sameColorBlock) {
		...
		for (Block enemyBlock : state.getEnemyBlocks()) {
			...
			removeBreathPoint(enemyBlock, state);
		...
		if (friendBlocks >= 2) {
			...
			for (Block friendBlock : neighborState.getFriendBlocks()) {
				...
				for (BlankBlock breathBlock : friendBlock.getBreathBlocks()) {
					if (breathBlock.removeNeighborBlock_oneWay(friendBlock) == false)
						throw new RuntimeException(
			...
			this.setBlock(point, sameColorBlock);
			if (log.isInfoEnabled()) {
				log.info("一.2 合并到唯一的相邻子块中。" + sameColorBlock.getBehalfPoint());
			}
			return sameColorBlock;
	...
	public void giveUp() {
		shoushu++;
		statistic.increaseGiveUpSteps();
		StepMemo step = new StepMemo(null,
				ColorUtil.getCurrentStepColor(shoushu));
		// step.setStep(step2);
		this.stepHistory.addStep(step);
	}
	...
	private void dealJie(Point point, byte jubutizishu, StepMemo step) {
		if (getBlock(point).getBreaths() == 1 && jubutizishu == 1
			...
			if (breathBlocks.size() == 1) {
				...
				Point tempPoint = ((breathBlocks.iterator().next()))
						.getUniquePoint();
				step.setProhibittedPoint(tempPoint);
				BoardPoint temp = this.getBoardPoint(tempPoint);
				temp.setProhibitStep((shoushu + 1),
						ColorUtil.enemyColor(getColor(point)));
				// 将互为打劫的两点联系在一起.
				this.getBoardPoint(point).setTwinForKo(tempPoint);
	...
	private void realDivideBlankPointBlock(NeighborState state) {
		...
		if (countNewBlank > 1) {
			if(log.isEnabledFor(Level.WARN)) log.warn("New blank block generated:" + newBlank.getTopLeftPoint());
	...
	public boolean isConnectedWithoutPoint(NeighborState state,
		...
		this.clearFlag();
		return count == blankBlock.getNumberOfPoint() - 1;
	}
	...
	/**
	 * 落子后,落子点周围的棋块是否仍和原气块相邻
	 * 
	 * @param block
	 *            落子点周围的棋块.可以是同色己方块,亦可以是异色敌方块.
	 * @param original
	 *            落子点
	 * @return
	 */
	private void removeBreathPoint(Block block, NeighborState state) {
		// 落子点原来所在的气块.
		Point original = state.getOriginal();
		BlankBlock blankBlock = state.getOriginalBlankBlock();
		block.removeBreathPoint(original);

		if (blankBlock.getNumberOfPoint() == 0) {
	...
	public void check(String temp) {
		CopyOfGoBoardForward goBoard = this;
		// Set<Block> whiteBlocks = goBoard.getWhiteBlocks();
		GoBoard goBoard2 = new GoBoard(this.boardSize);
	...
	protected void outputManualWithIssue() {
		GoManual goManual = new GoManual(this.boardSize);
