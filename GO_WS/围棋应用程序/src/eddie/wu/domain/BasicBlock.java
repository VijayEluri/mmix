...
public abstract class BasicBlock {
	...
	/**
	 * by default, it is active; it become inactive if it no longer exist in
	 * current board. it may become active again if one step backward later.
	 */
	...
	public Point getTopLeftPoint() {
		...
		if (list.isEmpty()) {
			if (this.getColor() == Constant.BLANK)
				return null;// empty breath block. disappear
			if (log.isDebugEnabled())
				log.debug("point list is empty!");
