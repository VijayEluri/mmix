...
public class TestConvertGoManual extends TestCase {
	private static final Logger log = Logger
			.getLogger(TestConvertGoManual.class);

	public void testConvert_sgf() {
		SimpleGoManual goManual = SGFGoManual
				.loadSimpleGoManual(Constant.currentManual);
		ConvertGoManual convert = new ConvertGoManual();
		// convert.convertFormat(row, column);
	}

	/**
	 * iterator all manuals. search the one has special characters.
	 * 
	...
	public void testConvert() {
		...
		byte[] temp = new LoadGMDGoManual("doc/围棋打谱软件/").loadSingleGoManual()
				.getMoves();
		...
		if (log.isDebugEnabled()) {
			for (int i = 0; i < temp.length / 2; i++) {
				if (log.isDebugEnabled())
					log.debug("i=" + i + ", [" + temp[2 * i] + ","
							+ temp[2 * i + 1] + "];" + " [" + result[2 * i]
							+ "," + result[2 * i + 1] + "]");
			}
		}
		int count = 0;
		// List<GMDGoManual> list = new
		// LoadGMDGoManual("doc/围棋打谱软件/").loadMultiGoManualFromLib0();

		byte[] original = null;

		for (int j = 1; j < 2; j++) {

			List<GMDGoManual> list = new LoadGMDGoManual("doc/围棋打谱软件/")
					.loadMultiGoManual(j);
			for (Iterator<GMDGoManual> iter = list.iterator(); iter.hasNext();) {
				count++;
				// if(count<760) continue;
				original = (byte[]) iter.next().getMoves();
	...
	public void testSimgleGoManual() {
		...
		byte[] temp = new LoadGMDGoManual("doc/围棋打谱软件/")
				.loadOneFromAllGoManual(1, 760);
		ConvertGoManual convert = new ConvertGoManual();
		byte[] result = convert.convertFormat(temp);
		for (int i = 0; i < temp.length / 2; i++) {
			if (log.isDebugEnabled())
