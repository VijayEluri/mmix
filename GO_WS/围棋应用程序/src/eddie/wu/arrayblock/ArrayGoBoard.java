 ...
 * </p>
 * 考虑做成一个研究用的界面工具，能生成棋谱，和局面。 仍然有实际意义，因为这样可以提高运行速度， 浪费一点内 存是值得的。
 * 2003年十月五日：增加周围块的信息。
 * <p>
 * Description:算气,处理提子. a,b表示的是落子点的横纵坐标，但是a，b同时作为行列坐标使用， 所以其实是 进行了一个映射。
 * </p>
 * <p>
 * 原先最多有128块棋，后来扩展到可以最多256块棋。
 * </p>
 * 可以以此实现作为性能的基准。
 */

public class ArrayGoBoard implements GoBoardInterface {
	...
	/**
	 * 每一块各自的坐标（ mei yi kuai de ge zi zuo biao）<br/>
	 ...
	 * 9~12 kuai suo ying of four eaten block<br/>
	 * 
	 * 13~24is same , 25,26a,b <br/>
	 * 记录棋局的过程信息,用于悔棋;<br/>
	 * 0=新块索引;<br/>
	 * 1-8为被吃单子的坐标;9-12为被吃块的索引;<br/>
	 * 13-20为成块子的坐标,21-24为成新块的旧块索引.<br/>
	 * 27-32为打吃点,33-35为打吃的块索引;<br/>
	...
	public void outputState(DataOutputStream jmout) throws IOException {
		byte i, j;
		for (i = 1; i < 20; i++) {
			for (j = 1; j < 20; j++) {
				if (zb[i][j][ZTXB] != 0) {
	...
	public void outputStateInTextFormat(DataOutputStream jmout)
			throws IOException {
		byte a, b;
		int count = 0;
		byte color;
	...
	/**
	 * transfer into two dimensions array
	 * 
	 * @return
	 */
	...
	public BoardColorState getBoardState() {
		...
		return new BoardColorState(this.getStateArray(),color);
	}
	...
	public ArrayGoBoard(int boardSize) {
		byte i, j;
	...
	public ArrayGoBoard(ArrayGoBoard goold) {
	...
	public Set<Point> getEatenPoints() {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (m != 0 && n != 0)
				points.add(Point.getPoint(boardSize, m, n));
			else
				break;
		}
		// 当前一步提吃的气块
