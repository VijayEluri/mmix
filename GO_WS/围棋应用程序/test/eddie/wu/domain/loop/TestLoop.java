...
public class TestLoop extends TestCase {
	...
	public void testLoop1() {
		...
		Assert.assertTrue(sa.isAlreadyLive_dynamic(target));
		Assert.assertTrue(sa.isStaticLive(target));
		target = sa.getPoint(3, 3);
		Assert.assertFalse(sa.isStaticLive(target));
		target = sa.getPoint(5, 5);
		Assert.assertFalse(sa.isStaticLive(target));
		target = sa.getPoint(6, 4);
		Assert.assertTrue(sa.isAlreadyDead_dynamic(target));
