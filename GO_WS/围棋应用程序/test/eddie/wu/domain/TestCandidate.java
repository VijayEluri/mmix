...
public class TestCandidate extends TestCase {
	...
	static {
		Logger.getLogger(Logger.class).setLevel(Level.WARN);
	}
	...
	public void testRealLifeProblem_20131393_5_six_3() {
		...
		List<Candidate> candidate = ta.getCandidate_forTarget(null, point, scope,
				Constant.WHITE, true, false);
		if (log.isEnabledFor(Level.WARN)) {
			log.warn(ta.getBoardColorState().getStateString());
			log.warn(candidate);
		}
		assertEquals(candidate.get(0).getStep().getPoint(), Point.getPoint(6, 2, 2));
	}
