...
public class TestAllState4New extends TestCase {

	private transient static final Logger log = Logger
			.getLogger(TestAllState4New.class);
...
//		}
	...
	public void testLive_Search() {
		...
		if(log.isEnabledFor(Level.WARN)) log.warn(sa.getBoardColorState().getStateString());
		Point point = Point.getPoint(4, 2, 1);
		Point blankP = Point.getPoint(4, 3, 2);
		BigEyeSearch goS = new BigEyeSearch(state, point,
		...
		if(log.isEnabledFor(Level.WARN)) log.warn(goS.getSearchProcess().size());
		Assert.assertEquals(RelativeResult.ALREADY_LIVE, score);
	}
	...
	public void testLive_Search222() {
		...
		if(log.isEnabledFor(Level.WARN)) log.warn(goS.getSearchProcess().size());
		Assert.assertEquals(RelativeResult.ALREADY_DEAD, score);
	}
	...
	public void testLive_Search_finalState3() {
		...
		Assert.assertFalse(live2);
		boolean dead2 = sa.isAlreadyDead_dynamic(point2);
		Assert.assertFalse(dead2);
	...
	//B-->[1,3]W-->[3,4]B-->[3,1]W-->[4,2]B-->[1,4]W-->[4,1]B-->[4,3]W-->[PAS]B-->[PAS](DB_PASS 3)
	...
	public void testFinalState_4() {
		...
		if(log.isEnabledFor(Level.WARN)) log.warn("state_deadCleanedUp=" + state_deadCleanedUp);

		boolean state_deadExist = ta.isFinalState_deadExist();

		if(log.isEnabledFor(Level.WARN)) log.warn("state_deadExist=" + state_deadExist);
	...
	public void testFinalState_41() {
		...
		if(log.isEnabledFor(Level.WARN)) log.warn("dynamic live=" + live);
		Assert.assertTrue(live);
