  ...
  public void init() {
    ...
    try {
      for (i = 1; i <= 7; i++) { //hong fang zai qian.
        ...
        yuansu[i] = createImage( (ImageProducer) url.getContent());
        imagename = "Blue" + (i - 1) + ".jpg";
        url = getClass().getResource(imagename);
        yuansu[7 + i] = createImage( (ImageProducer) url.getContent());
      }
      url = getClass().getResource("Board.jpg");
      yuansu[0] = createImage( (ImageProducer) url.getContent());
      url = getClass().getResource("pieceTex.jpg");
      yuansu[15] = createImage( (ImageProducer) url.getContent());
         }
         catch (Exception de) {
      de.printStackTrace();
         }
