...
public class ProducerConsumer {
	...
	public void demo() {
		...
		endAt = new long[capacity];
		for (int i = 0; i < threads; i++) {
			new Thread("[Producer Thread " + (i + 1) + "]") {
				// ThreadLocal local = new ThreadLocal();
				public void run() {
					produce();
					// endAt[i]=System.currentTimeMillis();
				}
			}.start();
	...
	public void consume() {
		...
		int beginIndex = threadName.indexOf("[") + 1;
		int endIndex = threadName.indexOf("]");
		int threadId = Integer.valueOf(
				threadName.substring(beginIndex, endIndex)).intValue();
		endAt[threadId]=System.currentTimeMillis();
	}
