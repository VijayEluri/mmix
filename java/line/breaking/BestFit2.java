...
/**
 * The difficulty is not to write code by following the instruction. Instead, the difficulty
 * is to distill the abstract from the instruction and make the design simple and clean. Only 
 * if we achieve this standard, we are willing to look at the code again someday later.
 */
import java.util.ArrayList;
import java.util.List;

/**
 * Copy from BestFit and rewrite. simplify and clean the code 
 */
public class BestFit2 extends TypeSet {
	public static boolean DEBUG = Constant.DEBUG;

	int lineWidth;

	public BestFit2(int lineWidth) {
	...
	List<List<Box>> breakPoints =new ArrayList<List<Box>>();
			...
			if (box instanceof Glue) {
				...
				if (width < lineWidth) {
					...
					if (rStretch <= 1) {// break here
						...
						breakPoints.add(lineBeforeWord);
					} else {
	...
	private void initParam() {
		...
		lineBeforeWord = null;
		this.betaAfter = 0;
		 breakPoints =new ArrayList<List<Box>>();
	}
